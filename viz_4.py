...
# -----------------------
def tasks_for_cpu(df, cpu_timeline):
    tasks = set()
...
# Single HTML
# -----------------------
def build_single_html(cpu_fig, life_fig, block_fig, cpu_tasks, cmap, df,
...
<body>
  ...
  <div class="card">
    ...
    {table_html}
    <div class="subtle">Use search + paging. DataTables is much faster than Plotly Table.</div>
  </div>
...
def main():
    ...
    cpu_tasks = tasks_for_cpu(df, cpu_tl)
    ...
    build_single_html(cpu_fig, life_fig, block_fig, cpu_tasks, cmap, df, out, xmin, xmax, dmax, args.table, args.slider_steps)

    print(f"✅ Wrote report: {out.resolve()}")
    print("Filters applied:")
    print(" - Lifecycle: removed esp_timer")
    print(" - Blocking: removed esp_timer and IDLE")
    if args.switch_markers:
        print(" - CPU: switch markers enabled")
